<template>
	<view class="container">
        ...
        <view class="header" v-if="essayContent">
          <view class="tit">{{ essayContent.link_title }}</view>
          <view class="info uni-flex">
            <view class=""><text>{{ essayContent.username }}</text><toDecima :str="essayContent.create_time" :type="1"></toDecima></view>
            <view class="opra-box uni-flex">
              <view class="btn-zan uni-flex" @click="clickZan(essayContent)"><image :src="essayContent.has_star ? '/static/images/icon/icon-zan-2.png' : '/static/images/icon/icon-zan-1.png'" mode=""></image>{{ essayContent.star >= 5000 ? '5000+' : essayContent.star }}</view>
              <view class="btn-copy uni-box uni-txc" @click="copy(essayContent.link_title, 1)">复制文案</view>
            </view>
          </view>
        </view>
        ...
        <view class="main" v-if="essayContent">
          <view>
          	<rich-text :nodes="essayContent.link_content"></rich-text>
          </view>
        </view>
        <view class="author" v-if="essayContent">
          <view class="author-con uni-flex">
        ...
        <view class="other uni-flex" v-if="essayContent">
          <text class="btn" v-if="essayContent.link_out" @click="read(essayContent.link_out)">阅读原文</text>
          <text>阅读 {{ essayContent.link_count ? essayContent.link_count : 0 }}</text>
        </view>
			</scroll-view>
		</view>
    <!-- 复制微信号码 S -->
      ...
      this.$http({
        ...
        cb: (err, data) => {
          if (!err && data.success === true) {
            ...
            this.essayContent.link_content = this.htmlDecode(this.essayContent.link_content)
            this.essayContent.link_content = this.essayContent.link_content.replace(/\<img/gi, '<img style="display:block;max-width:100%;height:auto" ')
            // 生成二维码
            if (this.isAuthor) {
...
<style lang="scss" scoped>
  @import '../../common/var';
  .canvas-qrocde{
    top: -10000upx; width: 364upx; height: 364upx; pointer-events: none;
  }
  .content{
    ...
    .other{
      ...
      .btn{
        margin-right: 40upx; color: $color;
      }
    }
  }
</style>
