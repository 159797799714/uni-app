					...
					<view class="form">
						...
						<label>
							<view class="password">
								...
								<input type="text" v-model="password" password="true" placeholder="请输入6-16位的密码" placeholder-style="color:#fff"/>
							</view>
              <view class="password">
              	<image src="../../static/images/login/lock1.png" mode=""></image>
              	<input type="text" v-model="password2" password="true" placeholder="请再次输入密码" placeholder-style="color:#fff"/>
						...
						<view class="register-now" :class="{'disabled': submiting}" @click="submit">立即注册</view>
						<view class="tologin" @click="tologin">已有账号？立即登录</view>
					</view>
  ...
  const COUNTSECEND = 60
    ...
    onShow() {
    	...
    	if (uni.getStorageInfoSync('registerStamp')) {
    	  let oldtime = uni.getStorageInfoSync('registerStamp')
...
<style lang="scss" scoped>
  ...
  .form{display: flex;flex-direction: column;padding:0px 49upx;
	  ...
	  .code{border: 0px;display: flex;margin-bottom: 28upx;
			...
			.code-btn{height: $height;color: $color-white; width: 209upx;border: $border;border-radius: $radius;line-height: $height; text-align: center;font-size: 29upx;font-family:PingFang-SC-Regular;
      .text{
		...
		.register-now{height: $height;background: #67C90A;text-align: center;line-height: $height;box-shadow:0px 1px 2px 0px rgba(0, 0, 0, 0.1), 0px 2px 0px 0px rgba(103,201,10,1);
		  border-radius:9upx;font-size: 35upx;color:$color-white;margin-top: 62upx;margin-bottom: 33upx;
		}
		.tologin{font-size: 33upx;text-decoration:underline;color:$color-white;text-align: right;}		
	}
	.footer{display: flex;flex-direction: column;align-items: center;position: absolute;bottom: 68upx;width: 100%;
	  text{font-size: 24upx;font-weight: bold;color: $color-white;margin-bottom: 41upx;}
		.weixin{height: 88upx;width: 88upx;}
	}
</style>
